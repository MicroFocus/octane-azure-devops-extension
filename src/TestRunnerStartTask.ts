...
export class TestRunnerStartTask extends BaseTask {
  ...
  protected prepareAzureVariables() {
    this.collectionUri = this.tl.getVariable(
    ...
    this.jobStatus = this.convertJobStatus(
      this.tl.getVariable("AGENT_JOBSTATUS")
    );
  ...
  private async initTestRunnerVariables() {
    await new Promise<void>(async (resolve, reject) => {
      ...
      this.tl.setResult(
        this.tl.TaskResult.Failed,
        "Tests Converter initialization failed"
      );
  ...
  protected async getExecutor(
    ...
    let executors;
    const executorQuery = Query.field("ci_id")
      .equal(BaseTask.escapeOctaneQueryValue(this.getJobCiId()))
      .and(
        Query.field(EntityTypeConstants.CI_SERVER_ENTITY_TYPE).equal(
          Query.field("id").equal(ciServer.id)
        )
      )
      .build();
    ...
    const ciJobs = await octaneSDKConnection
      .get(EntityTypeRestEndpointConstants.CI_JOB_REST_API_NAME)
      .fields(
        "name,ci_server{server_type},name,ci_id,parameters,executor{id, subtype}"
      )
      .query(executorQuery)
      .execute();
    ...
    if (ciJobs && ciJobs.total_count > 0 && ciJobs.data) {
      ...
      executors = ciJobs.data
        .filter((ciJob) => ciJob.executor)
        .map((ciJob) => ciJob.executor);
    } else {
      await this.createCiJob(octaneSDKConnection);
    }
    if (!executors || executors.length == 0) {
      // If missing executor - create
        if (this.framework === 'uft') {
            return await this.createUftExecutor(octaneSDKConnection);
        } else {
  ...
  protected async createCiJob(octaneSDKConnection) {
    ...
    const ciJobs = [
      await octaneSDKConnection
        .create(EntityTypeRestEndpointConstants.CI_JOB_REST_API_NAME, ciJob)
        .execute(),
    ];
  ...
  private async createUftExecutor(octaneSDKConnection) {
      const githubRepoUrl = this.tl.getInput(InputConstants.REPO_URL);
      if (!githubRepoUrl) {
            this.logger.error("GitHub repository URL is required to create UFT executor.");
            return;
      }
      ...
      const executors = [await octaneSDKConnection.executeCustomRequest(`/internal-api/shared_spaces/${this.sharedSpaceId}/workspaces/${this.workspaces[0]}/je/test_runners/uft`,
                         Octane.operationTypes.create, uft_test_runner)];

      this.logger.info("UFT Executor creation response: " + JSON.stringify(executors));
  ...
  private async createExecutor(octaneSDKConnection) {
    ...
    const executors = [
        await octaneSDKConnection
            .create(
                EntityTypeRestEndpointConstants.EXECUTORS_REST_API_NAME,
                test_runner
            )
            .execute(),
    ];
  ...
  private async getFrameworkId(framework) {
    switch (framework) {
      ...
      case "junit":
        return "list_node.je.framework.junit";
      case "uft":
        return "list_node.je.framework.uft";
      default:
  ...
  public async run() {
    ...
    if (executionId && suiteRunId) {
      ...
      this.logger.info(
        "there is no execution id (meaning - no suite run from Octane), no need to send event"
      );
    }
  }
  ...
  private async runInternal() {
    ...
    const convertedTests = converter.convert(
      this.testToConvert,
      this.framework
    );

      this.logger.info("Tests to run after conversion: " + convertedTests);
