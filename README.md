...
> [!CAUTION]
> For successful test and test results injections, in case of UFT One tests, an additional parameter/variable must be set in the End task. Please refer to Chapter 7 ([Chapter 7.](#7-displaying-junituft-onenunit-test-results-into-the-product)).
...
> [!CAUTION]
> Starting with version `26.3.0`, a new method has been introduced that allows you to display test results into the product. The modifications 
> are MANDATORY for the UFT One test results injection flow, but are OPTIONAL for the other frameworks. You can follow the steps below to display 
> test results in the product.
...
To enable coverage injection using JaCoCo or LCOV, you need to do two things:
1. **Publish the generated report file as a build artifact**
2. **Select the report type in the `octane-end-task` configuration**

The publishing of the report file as a build artifact is necessary , so that the extension will be able to download and send it to the product. You can do that
by using the `PublishBuildArtifacts` task in your pipeline, as shown in the example below, where `PathToPublish` should point to the location of the generated report file.
Here is a sample configuration for JaCoCo:
...
> [!IMPORTANT]
> The artifact name **needs to be** left as `coverage-report`, as this is the name that the extension looks for when trying to find the report file.

> [!IMPORTANT]
> The published JaCoCo XML file must follow the naming pattern `**/jacoco*.xml` (or for lcov `**/lcov*.info`). The extension searches for the coverage report using this glob pattern.
Renaming the file to a custom name that does not contain jacoco may prevent the extension from detecting it.

After publishing the report as an artifact, you need to select the report type in the `octane-end-task` configuration, by filling in the `CodeCoverageReportType` field 
with either `jacoco` or `lcov`, depending on the type of report you are generating. The extension will then look for a build artifact with the name `coverage-report`,
and if found, it will try to find a file matching the glob pattern for the corresponding report type, and if such file is found, it will be injected into the product.
You should end up with a configuration similar to the one below:

```yaml
- task: octane-end-task@26
  inputs:
...
The pipeline must include the following SonarQube tasks:

- `SonarQubePrepare@<version>`
- `SonarQubeAnalyze@<version>` (if applicable for the selected scanner mode)
- `SonarQubePublish@<version>`

These tasks are responsible for:

- Preparing the analysis configuration
- Executing the scan (depending on scanner mode)
- Publishing the analysis results to SonarQube

The coverage injection feature relies on an already completed and published SonarQube analysis. No additional coverage report artifacts need to be published for this mode.
...
- provide a SonarQube service connection
- provide the project-key in the `SonarQubePrepare@<version>` task.

The first two elements must be configured inside the `octane-end-task` as shown in the example below, where `sonarConnectionName` is the name of the SonarQube service 
connection configured in Azure DevOps, and `SONAR_REPORT` is selected as the `CodeCoverageType`:
...
- task: octane-end-task@26
  inputs:
    ...
    WorkspaceList: '2003'
    Framework: 'junit'
    CodeCoverageType: 'SONAR_REPORT'
    SonarQubeServiceConnection: 'sonarConnectionName'
  env:
    UNIT_TEST_RESULTS_GLOB_PATTERN: ${{ parameters.unitTestResultsGlobPattern }}
```

The `projectKey` must be configured depending on the selected scanner mode. 

**Maven Scanner Mode**:
```yaml
- task: SonarQubePrepare@5
  inputs:
    SonarQube: 'sonarConnectionName'
    scannerMode: 'Other'
    extraProperties: |
...
- task: SonarQubePrepare@5
  inputs:
    ...
    scannerMode: 'CLI'
    configMode: 'manual'
    cliProjectKey: 'my-cli-project'
```

The SonarQube service connection is required because the `octane-end-task` retrieves coverage data directly from the SonarQube REST API.
The already configured sonar service connection provides the authentication token used to authorize this request.
...
Now whenever you run any pipeline and check the logs, you will notice that there are additional log messages, compared to previous runs, distinguished by the color purple:
